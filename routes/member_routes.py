...
def router_create_member(body: Member):
    ...
    try:
        ...
        return {"massage": f"Member ID: {new_id} Added successfully"}
...
@router.patch("/{id}")
def router_update_membe(id: int, body: UpdatedMember):
    ...
    if changed:
        ...
        return {"massage": f"Member ID: {id} updated successfully"}
...
def router_deactivate_member(id: int):
    ...
    if changed:
        ...
        return {"massage": f"The friend id:{id}was successfully neutralized."}
    else:
        logging.error(f"No member found with ID:{id}")
        raise HTTPException(status_code=404, detail=f"No member found with ID:{id}")
...
def router_activate_memberr(id: int):
    ...
    if changed:
        ...
        return {"massage": f"The friend id:{id}was successfully activated."}
    else:
        logging.error(f"No member found with ID:{id}")
        raise HTTPException(status_code=404, detail=f"No member found with ID:{id}")
